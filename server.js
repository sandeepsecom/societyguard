...
app.get("/api/stats", requireApiKey, async (req, res) => {
  ...
  try {
    const [todayR,yestR,weekR,camsR,downR,countR] = await Promise.all([
      ...
      pool.query(`SELECT camera_id,camera_location as location,COUNT(*) as count FROM events WHERE timestamp_utc>=NOW()-INTERVAL '7 days' ${cid} GROUP BY camera_id,camera_location ORDER BY count DESC`),
